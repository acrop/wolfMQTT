...
int mqtt_publish_msg(MQTTCtx *mqttCtx, const char* topic, MqttQoS qos, word32 timeout_ms, const uint8_t *content, int len)
{
    ...
    for (;;) {
      ...
      /* Only when not sending ping and there is no publish resp need to send,
            we can publish message */
      if ((mqttCtx->stat == WMQ_WAIT_MSG) &&
        !client->ping_sending &&
        (resp_body_head->resp_type == MQTT_PACKET_TYPE_RESERVED)) {
        break;
      }
...
int mqtt_receive_msg(struct ringbuf *on_message_rb, uint8_t *buffer, uint32_t buffer_len)
{
    uint32_t total_len;
    size_t sizeof_total_len = sizeof(total_len);
    size_t readed_len = ringbuf_read(on_message_rb, (uint8_t *)&total_len, sizeof_total_len, false);
    if (readed_len != sizeof_total_len) {
        if (readed_len == 0) {
            return 0;
        }
        ringbuf_reset(on_message_rb);
        return -1;
    }
    ...
    if (total_len > on_message_rb->capacity) {
        ringbuf_reset(on_message_rb);
        return -3;
    }

    if (ringbuf_read_available(on_message_rb) < total_len) {
        return 0;
    }
    if (buffer_len < total_len) {
        ringbuf_read(on_message_rb, NULL, total_len, true);
        return -5;
    } else {
        ringbuf_read(on_message_rb, buffer, total_len, true);
        return 1;
    }
    return 0;
}
