...
static int MqttClient_SendObjectWaitType(MqttClient* client, MqttSendObjectOption *option)
{
    ...
    if (rc == MQTT_CODE_SUCCESS && obj->stat.on_wait_type) {
        ...
        if (obj == &client->msg) {
            ...
            obj->stat.start_time_ms = client->read_time_ms;
        } else {
        #ifdef WOLFMQTT_MULTITHREAD
            rc = MqttClient_WaitAck(client, obj,
                option->ack_packet_type, option->packet_id, option->timeout_ms);
        #else
            rc = MQTT_CODE_ERROR_STAT;
        #endif
        ...
        int timeout_ms)
{
    ...
    if (client->net->get_timer_ms != NULL) {
        ...
        int check_rc = MqttClient_CheckTimeout(MQTT_CODE_CONTINUE, &client->start_time_ms,
        ...
        check_rc = MqttClient_CheckTimeout(
            MQTT_CODE_CONTINUE, &client->read_time_ms,
    ...
    while (rc == MQTT_CODE_SUCCESS) {
        ...
        if (client->ping_started == 0 && resp_body_head->resp_type != MQTT_PACKET_TYPE_RESERVED) {
            ...
            rc = MqttClient_SendPublishResp(
                client, &client->publish_resp, resp_body_head->resp_type,
