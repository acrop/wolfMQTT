    ...
    MqttMsgState next_stat) /* The next stat after writing header or payload finished */
{
    /* Lock send socket mutex */
    int rc = MQTT_CODE_SUCCESS;
    /* Network is either disconnected, or not connect yet */
    ...
    case MQTT_MSG_BEGIN:
    {
        ...
        if (next_stat != MQTT_MSG_BEGIN) {
            ...
            /* The lock acquired, need update read/write time for not raise timeout error */
...
static void MqttClient_CheckPingTimeout(MqttClient *client)
{
    if (client && client->net.get_time_ms) {
        /* Only non-blocking mode can check the timeout properly */
        word32 now_ms = client->net.get_time_ms();
        /* Checking the socket read timeout for ping */
        byte read_timeout = MqttClient_CheckTimeout(client->time_socket_read_ms, client->keep_alive_ms, now_ms);
        /* Checking the socket write timeout for ping */
        byte write_timeout = MqttClient_CheckTimeout(client->time_socket_write_ms, client->keep_alive_ms, now_ms);
        /* just need send ping when read or write when in non-blocking mode */
        if (read_timeout || write_timeout) {
            /* The ackQueue should be locked by recv lock */
            int rc = MqttMsgPacketHeaderQueue_Write(&client->ackQueue, MQTT_PACKET_TYPE_PING_REQ, 0, 0);
            if (rc == MQTT_CODE_SUCCESS) {
                /**
                 * Directly append the ping to the client->ackQueue for
                 * non-blocking mode so we won't need call MqttClient_Ping in state machine
                 * or other thread, if append failed, that's means the ackQueue is full,
                 * then try it latter, that won't cause failure.
                 */
                client->time_socket_read_ms = now_ms;
                client->time_socket_write_ms = now_ms;
            }
...
static int MqttClient_WaitAck(int send_rc, MqttClient *client,
    void *packet_obj_send, int timeout_ms)
{
    MqttMsgHeader *msgHeader = (MqttMsgHeader *)packet_obj_send;
    int rc = MQTT_CODE_SUCCESS;
    if (send_rc != MQTT_CODE_SUCCESS && send_rc != MQTT_CODE_CONTINUE) {
        /* It's already failed when sending the packet */
        return send_rc;
    }
    do {
        ...
        if (client->useNonBlockMode) {
            /**
             * Non-blocking mode, we can raise ping timeout for sending ping.
             * Also we can receiving more data when publish qos0 data in non-blocking mode.
             * So that in non-blocking state machine, we can receiving message when publish
             * qos0 data. So that we won't break the loop in non-blocking mode; so that
             * in non-blocking mode, when the data is partial sent, we can still receiving data
             * from the broker in parallel
             */
            if (wait_ack || no_ack) {
                /* Only non-blocking mode can check the timeout for the command properly */
                word32 now_ms = client->net.get_time_ms();
                /* Checking the socket read timeout for this command */
                byte read_timeout = MqttClient_CheckTimeout(client->time_socket_read_ms, timeout_ms, now_ms);
                /* Checking the socket write timeout for this command */
                byte write_timeout = MqttClient_CheckTimeout(client->time_socket_write_ms, timeout_ms, now_ms);
                /* Both read and write are timeout, then raise timeout error for waiting ack */
                if (read_timeout && write_timeout) {
            ...
            if (no_ack) {
                /**
                 * publish qos0 and disconnect packet can not wait `ack` or `any`
                 * in blocking mode, and also can not detecting ping timeout in blocking
                 * mode, so we break at this condition
                 */
                break;
            }
        }
        ...
        /* The ping timeout check should handle inside receive lock, as the ping will append to ackQueue */
        MqttClient_CheckPingTimeout(client);

        /* The recv lock is acquired */
        do {
            if (wait_ack && msgHeader->packetDone) {
                /**
                 * If we are waiting an ack and the ack is received
                 * or failed with error, then break the loop,
                 * and do not call MqttClient_WaitType any more.
                 */
                break;
            }

            /* Only the command is fully sent, we can sending the ack */
            if (send_rc == MQTT_CODE_SUCCESS) {
                MqttMsgPacketHeader *new_resp = MqttMsgPacketHeaderQueue_Read(&client->ackQueue);
                if (new_resp != NULL) {
                    /* avoid unnecessary lock on send */
                    /* TODO: how to avoid dead loop here for non-blocking mode */
                    rc = MqttClient_SendLock(client, &resp->header);
                    while (rc == MQTT_CODE_SUCCESS && new_resp != NULL) {
                        /* The send lock is acquired */
                        resp->header.packet = *new_resp;
                        /* Write the ack packet */
                        do {
                            rc = MqttClient_WriteEncoded(client, &resp->header,
                                resp->header.packet.type, MQTT_PACKET_TYPE_RESERVED,
                                resp->header.packet.id, NULL, MQTT_MSG_BEGIN);
                        } while (rc == MQTT_CODE_CONTINUE);
                        /* Reset the resp packet for next ack write */
                        XMEMSET(&resp->header.packet, 0, sizeof(resp->header.packet));
                        new_resp = MqttMsgPacketHeaderQueue_Read(&client->ackQueue);
                    }
            ...
            if (wait_any) {
                if (client->useNonBlockMode) {
                    ...
                    /* Only receive a single message for wait `any` in blocking mode, so we break the loop */
                }
            } else if (wait_ack) {
                if (rc == MQTT_CODE_SUCCESS) {
                ...
                /* It's no ack, there is no need waiting anymore, so break it */
            }
        } while (0);
...
#ifdef WOLFMQTT_V5
    /* Enhanced authentication */
    if (rc == MQTT_CODE_SUCCESS && mc_connect->header.writeState == MQTT_MSG_AUTH) {
        if (mc_connect->header.protocol_level <= MQTT_CONNECT_PROTOCOL_LEVEL_4) {
    ...
    MqttPublish* publish, int timeout_ms)
{
    ...
    for (;;) {
        ...
        /**Issue callback for message,
         * for done message, always calling the callback,
         * as 0 length payload message are allowed in mqtt,
         * for not done message, only the buffer_len > 0 are allowed
         */
        if (client->msg_cb && (msg_done || publish->buffer_len > 0)) {
...
int MqttClient_WaitMessage(MqttClient *client)
{
    ...
    return MqttClient_WaitAck(MQTT_CODE_SUCCESS, client, NULL, client->keep_alive_ms);
}
...
{
    int rc = MqttSocket_Disconnect(client);
    if (rc == MQTT_CODE_CONTINUE) {
        return rc;
    }
    MqttClient_RespListClear(client);
#ifdef WOLFMQTT_MULTITHREAD
    /* Make sure all send thread finish their sending of data and to clear pending list */
    rc = wm_SemLock(&client->lockSend);
    if (rc < 0) {
        return rc;
    }
    wm_SemUnlock(&client->lockSend);
#endif
    /* Waiting other thread to clean up the the pending list */
    for (;;) {
        /* No need judge the result of MqttClient_WaitAck, it's always MQTT_CODE_ERROR_NETWORK */
        MqttClient_WaitAck(MQTT_CODE_SUCCESS, client, NULL, 0);
        if (client->firstPendResp == NULL && client->lastPendResp == NULL)
        {
            return MQTT_CODE_SUCCESS;
        }
        if (client->useNonBlockMode) {
